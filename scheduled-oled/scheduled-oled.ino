...
const int oledBrightnessInterval = 300000;
unsigned long previousOledBrightness = 0;
...
void updateOledBrightness() {
  if (currentMillis - previousOledBrightness >= oledBrightnessInterval){
    previousOledBrightness += oledBrightnessInterval;
    ...
    // Check if the current hour is before 8
    if(currentHour <= 8){
      u8g2.setPowerSave(1);
    } else{
      u8g2.setPowerSave(0);
